...
export default function Hero() {
  ...
  const socialLinks = [
    ...
    { href: 'mailto:[EMAIL_ADDRESS]', icon: <FaEnvelope />, label: 'Email', color: 'hover:text-emerald-400 hover:bg-emerald-900/30 hover:border-emerald-500/50' }
  ]
...
=======
              {socialLinks.map((link, idx) => (
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className={`pointer-cursor p-3 rounded-xl text-lg text-slate-400 bg-white/5 border border-white/10 backdrop-blur-sm transition-all duration-300 hover:-translate-y-1 ${link.color}`}
                  title={link.label}
                >
                  {link.icon}
                </a>
